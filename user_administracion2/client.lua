...
RegisterNUICallback("ban", function(data,cb)
	...
	ESX.TriggerServerCallback("el_bwh:ban",function(success,reason)
		if success then ESX.ShowNotification("~g~Jugador expulsado con éxito") else ESX.ShowNotification(reason) end -- dont ask why i did it this way, im a bit retarded
...
RegisterNUICallback("warn", function(data,cb)
	...
	ESX.TriggerServerCallback("el_bwh:warn",function(success)
		if success then ESX.ShowNotification("~g~Jugador advertido con éxito") else ESX.ShowNotification("~r~Algo salió mal") end
...
RegisterNUICallback("unban", function(data,cb)
	...
	ESX.TriggerServerCallback("el_bwh:unban",function(success)
		if success then ESX.ShowNotification("~g~Jugador desbloqueado con éxito") else ESX.ShowNotification("~r~Algo salió mal") end
	end, data.id)
end)
...
AddEventHandler("el_bwh:receiveWarn",function(sender,message)
	TriggerEvent("chat:addMessage",{color={255,255,0},multiline=true,args={"[Shop Store Administracion]","Recibiste una advertencia"..(sender~="" and " de "..sender or "").."!\n-> "..message}})
	Citizen.CreateThread(function()
		local scaleform = RequestScaleformMovie("mp_big_message_freemode")
		while not HasScaleformMovieLoaded(scaleform) do Citizen.Wait(0) end
		BeginScaleformMovieMethod(scaleform, "SHOW_SHARD_WASTED_MP_MESSAGE")
